 ...
 */
public class TrailerComparator extends AbstractComparator {

    public TrailerComparator() {
    }

    public TrailerComparator(DataDictionary inDictionary, List<String> inMessageErrors) {
        super(inDictionary, inMessageErrors);
    }

    public TrailerComparator(DataDictionary inDictionary, String inMessageType, List<String> inMessageErrors) {
    ...
    public boolean compare(final FixMessageType inFixModel) {
        ...
        for (final FixMessageType.Field f : fields) {
            ...
            if (!dictionary.isTrailerField(fieldId)) {
                continue;
            }

            final FixMessageType.Field modelField = FixUtils.getModelField(fieldId, inFixModel.getFields(), false);
            if (modelField == null) {
                messageErrors.add("Trailer field [" + f.getName() + ", " + f.getValue() + "] does not exist in received message");
            } else {
