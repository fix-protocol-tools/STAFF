...
//    private static final Map<SessionID, Session> sessions = new HashMap<SessionID, Session>();
    ...
    Session(Application application, MessageStoreFactory messageStoreFactory, SessionID sessionID,
        ...
        if (application instanceof IMessageFilterContainer) {
            rawFilterOut = ((IMessageFilterContainer) application).getRawFilterOut();
            rawFilterIn = ((IMessageFilterContainer) application).getRawFilterIn();

            fixFilterOut = ((IMessageFilterContainer) application).getFixFilterOut();
            fixFilterIn = ((IMessageFilterContainer) application).getFixFilterIn();
        }
    }
            ...
            for (final String message : messages) {
                ...
                if (MessageUtils.isAdminMessage(msgType) && !forceResendWhenCorruptedStore) {
                    ...
                    if (resendApproved(msg)) {
                        ...
                        // it real need send via filters?
//                        send(msg.toString());
                        sendViaFixFilter(msg, extractSeqNum(msg));
    ...
    private Message parseMessage(String messageData) throws InvalidMessage {
        return MessageUtils2.parse(this, messageData);
    }
    ...
    private boolean verify(Message msg, boolean checkTooHigh, boolean checkTooLow)
        ...
        try {
            ...
            if (!validLogonState(msgType)) {
                throw new LogonStateException("Logon state is not valid for message (MsgType="
                        + msgType + ")");
            }
            ...
            throw e;
        } catch (final LogonStateException e) {
    ...
    public void generateTestRequest(String id) {
        ...
        sendViaFixFilter(testRequest, 0);
    }
    ...
    private boolean generateLogon() throws IOException {
        ...
        return sendViaFixFilter(logon, 0);
    }
    ...
    private void sendResendRequest(String beginString, int msgSeqNum, int beginSeqNo, int endSeqNo) {
        ...
        sendViaFixFilter(resendRequest, 0);
    ...
    private boolean sendMsgWithoutSeqNumLock(final Message message, final int num, final boolean nextSeqNumBaseThis) {
        // sequence number must be locked until application
        // callback returns since it may be effectively rolled
        // back if the callback fails.
        try {
            ...
            if (message.isAdmin()) {
                ...
                if (msgType.equals(MsgType.LOGON) || msgType.equals(MsgType.LOGOUT)
                    ...
                    result = sendRawMsg(messageString);
                }
            } else {
                try {
                    application.toApp(message, sessionID);
            ...
            } else if (nextSeqNumBaseThis) {
                if (persistMessages) {
                    state.set(num, messageString);
                }
                state.getMessageStore().setNextSenderMsgSeqNum(num + 1);
            }
            return result;
        } catch (IOException e) {
    ...
    public boolean send(Message message) {
        ...
        return sendViaFixFilter(message, 0);
    }
    ...
    public boolean sequenceReset(int newSeqNum) throws IOException {
        boolean sendResult = false;
        state.lockSenderMsgSeqNum();
        try {
            Message sequenceReset = messageFactory.create(sessionID.getBeginString(), MsgType.SEQUENCE_RESET);
            final Header header = sequenceReset.getHeader();
            initializeHeader(header);
            sequenceReset.setInt(NewSeqNo.FIELD, newSeqNum);
            String messageString = sequenceReset.toString();
            sendRawMsg(messageString);
            getLog().onEvent("Sent SequenceReset TO: " + newSeqNum);
            if (persistMessages) {
                state.set(newSeqNum, sequenceReset.toString());
            }
            state.getMessageStore().setNextSenderMsgSeqNum(newSeqNum);
        } finally {
            state.unlockSenderMsgSeqNum();
        }
        return sendResult;
     ...
     * @return
     */
    public boolean sendAsRaw(final String message) {
        state.lockSenderMsgSeqNum();
        try {
            boolean sendResult = sendViaRawFilter(message);
            if (sendResult) {
                try {
                    state.incrNextSenderMsgSeqNum();
                } catch (IOException ioe) {
    ...
    private boolean send(final String inRawMessage) {
        return sendViaRawFilter(inRawMessage);
    }

    private boolean sendViaRawFilter(final String inRawMessage) {
        List<String> filteredMessage = Arrays.asList(inRawMessage);
        try {
            if (rawFilterOut != null && rawFilterOut.isActive()) {
                filteredMessage = rawFilterOut.filter(sessionID, inRawMessage);
            }
            for (final String rawMsg : filteredMessage) {
                // TODO: check it! if some of message has not been sent - return false
                if (!sendRawMsg(rawMsg)) {
                    return false;
                }
            }
            return true;
            ...
            if (log.isTraceEnabled()) {
                log.trace(sessionID + "msg skip: " + inRawMessage);
            }
        // TODO: check this functional - if we skip message, should we return true/false
            return true;
        }
    }

    private boolean sendRawMsg(final String rawMsg) {
        getLog().onOutgoing(rawMsg);
        if (log.isDebugEnabled()) {
            log.debug("SEND=" + rawMsg);
        }
        synchronized (responderSync) {
            if (!hasResponder()) {
                getLog().onEvent("Attempt to send while not connected (message stored until connected).");
                return false;
            }
            return getResponder().send(rawMsg);
        }
    }
    ...
    public void setRawFilterIn(final IRawMsgFilterIn inRawFilterIn) {
        rawFilterIn = inRawFilterIn;
    }

    public void setRawFilterOut(final IRawMsgFilterOut inRawFilterOut) {
        rawFilterOut = inRawFilterOut;
    }

    public void setFixFilterIn(final IFixMsgFilterIn inFixFilterIn) {
        fixFilterIn = inFixFilterIn;
    }

    public void setFixFilterOut(final IFixMsgFilterOut inFixFilterOut) {
    ...
    private boolean isRawFilterOutEnabled() {
        return rawFilterOut != null && rawFilterOut.isActive();
    }

    private boolean isFixFilterOutEnabled() {
        return fixFilterOut != null && fixFilterOut.isActive();
    }
    ...
    private class LogonStateException extends SessionException {
        private LogonStateException() {
        }

        private LogonStateException(String message) {
            super(message);
        }

        private LogonStateException(String message, Throwable cause) {
            super(message, cause);
        }

        private LogonStateException(Throwable cause) {
            super(cause);
        }
    }
}
