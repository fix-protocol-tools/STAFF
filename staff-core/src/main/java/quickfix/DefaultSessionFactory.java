...
public class DefaultSessionFactory implements SessionFactory {
    ...
    public Session create(SessionID sessionID, SessionSettings settings) throws ConfigError {
        try {
            ...
            DefaultApplVerID senderDefaultApplVerID = getSenderDefaultApplVerID(sessionID, settings);

            DefaultDataDictionaryProvider dataDictionaryProvider = getDataDictionaryProvider(sessionID, settings);
    ...
    public DefaultDataDictionaryProvider getDataDictionaryProvider(SessionID sessionID, SessionSettings settings) throws ConfigError {
        try {
        ...
        } catch (FieldConvertError err) {
            throw new ConfigError(err.getMessage(), err);
        }
    }

    public DefaultApplVerID getSenderDefaultApplVerID(SessionID sessionID, SessionSettings settings) throws ConfigError {
        if (sessionID.isFIXT()) {
            ...
            try {
                return new DefaultApplVerID(toApplVerID(
                        settings.getString(sessionID, Session.SETTING_DEFAULT_APPL_VER_ID))
                        .getValue());
            } catch (FieldConvertError fieldConvertError) {
                throw new ConfigError(fieldConvertError.getMessage(), fieldConvertError);
            }
